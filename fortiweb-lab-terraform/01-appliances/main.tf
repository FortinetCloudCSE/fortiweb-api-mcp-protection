...
data "azurerm_virtual_network" "vnet" { name = var.vnet_name resource_group_name = var.resource_group_name }
...
data "azurerm_subnet" "protected" { name = "snet-protected-10-10-2" virtual_network_name = var.vnet_name resource_group_name = var.resource_group_name }
...
resource "azurerm_network_interface" "fg_outside" {
  ...
  ip_configuration { name = "outside" subnet_id = data.azurerm_subnet.client.id private_ip_address_allocation = "Static" private_ip_address = "10.10.3.101" primary = true }
  dynamic "ip_configuration" {
    for_each = ["10.10.3.150", "10.10.3.151", "10.10.3.152", "10.10.3.153"]
    content { name = "vip-${replace(ip_configuration.value, ".", "-")}" subnet_id = data.azurerm_subnet.client.id private_ip_address_allocation = "Static" private_ip_address = ip_configuration.value }
  }
}
resource "azurerm_network_interface" "fg_inside" {
  ...
  resource_group_name = data.azurerm_resource_group.rg.name
  enable_ip_forwarding = true
  ip_configuration { name = "inside" subnet_id = data.azurerm_subnet.protected.id private_ip_address_allocation = "Static" private_ip_address = "10.10.2.101" primary = true }
}
...
resource "azurerm_network_interface" "fweb_protected" {
  ...
  resource_group_name = data.azurerm_resource_group.rg.name
  enable_ip_forwarding = true
  ip_configuration { name = "protected" subnet_id = data.azurerm_subnet.protected.id private_ip_address_allocation = "Static" private_ip_address = "10.10.2.100" primary = true }
  dynamic "ip_configuration" {
    for_each = ["10.10.2.150", "10.10.2.151", "10.10.2.152", "10.10.2.153"]
    content { name = "vip-${replace(ip_configuration.value, ".", "-")}" subnet_id = data.azurerm_subnet.protected.id private_ip_address_allocation = "Static" private_ip_address = ip_configuration.value }
  }
}
resource "azurerm_network_interface" "fweb_server" {
  ...
  resource_group_name = data.azurerm_resource_group.rg.name
  enable_ip_forwarding = true
  ip_configuration { name = "server" subnet_id = data.azurerm_subnet.server.id private_ip_address_allocation = "Static" private_ip_address = "10.10.1.100" primary = true }
}
...
resource "azurerm_linux_virtual_machine" "fortigate" {
  ...
  plan { publisher = var.fortigate_publisher product = var.fortigate_offer name = var.fortigate_sku }
  source_image_reference { publisher = var.fortigate_publisher offer = var.fortigate_offer sku = var.fortigate_sku version = var.fortigate_version }
...
resource "azurerm_linux_virtual_machine" "fortiweb" {
  ...
  plan { publisher = var.fortiweb_publisher product = var.fortiweb_offer name = var.fortiweb_sku }
  source_image_reference { publisher = var.fortiweb_publisher offer = var.fortiweb_offer sku = var.fortiweb_sku version = var.fortiweb_version }
  os_disk { caching = "ReadWrite" storage_account_type = "Premium_LRS" }
